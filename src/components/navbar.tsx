...
export function Navbar({ left }: { left?: ReactNode } = {}) {
  return (
    <nav className="absolute inset-x-0 top-0 z-40 flex h-[62px] items-center justify-center px-5">
      {left && (
        <div className="absolute left-6 top-1/2 -translate-y-1/2">{left}</div>
      )}
      ...
      <ThemeToggle className="absolute right-5 top-1/2 -translate-y-1/2 cursor-pointer rounded-full p-2 text-primary opacity-(--opacity-muted) transition-opacity duration-140 hover:opacity-100" />
    </nav>
  )
}
