...
function MermaidComponent({ svg, code }: { svg?: string; code: string }) {
  if (!svg) {
    return (
      <pre className="post-media post-media--mermaid-fallback">
        <code>{code}</code>
      </pre>
    )
  }

  // SVG is generated by mermaid.js from trusted author-written definitions,
  // same trust model as the compiled HTML pipeline.
  return (
    <div
      className="post-media post-media--mermaid"
...
export function ComponentResolver({ entry }: { entry: ComponentEntry }) {
  ...
  // Mermaid and svg-board have no media src — handle before mediaUrl
  if (entry.type === 'mermaid') {
    return <MermaidComponent svg={entry.props.svg} code={entry.props.code} />
  }
