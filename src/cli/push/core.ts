...
export async function pushCore(opts: {
  db: Db
  r2: R2Bucket
  kv: KVNamespace
  postsDir: string
}): Promise<PushResult> {
  const { db, r2, kv, postsDir } = opts

  // Phase 1: Scan + Validate
  const scanned = await scanPosts(postsDir)
  if (!validateAll(scanned)) {
    throw new PushValidationError('one or more posts failed validation')
  }
